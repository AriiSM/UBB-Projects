...
public class ServiceProxy implements IServices {
    ...
    public Iterable<JucatorGame> findAllJucatoriGames() throws GenericException {
        Request request = new Request(RequestType.GET_ALL_JUCATORI_GAMES, "get all jucatori games");
        sendRequest(request);
        Response response = readResponse();
        if(response.getType() == ResponseType.OK){
    ...
    public Configuratie saveConfiguratie(Configuratie configuratie) throws GenericException {
        Request request = new Request(RequestType.SAVE_CONFIGURATIE, configuratie);
        sendRequest(request);
        Response response = readResponse();
        if(response.getType() == ResponseType.OK){
    ...
    public JucatorGame saveJucatoriGames(JucatorGame jucatorGame) throws GenericException {
        ...
        Response response = readResponse();
        if(response.getType() == ResponseType.OK){
            return (JucatorGame) response.getData();
        }
        return null;
    }
}
