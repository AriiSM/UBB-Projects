...
public class MyRestClient {
    ...
    public String testGreeting(String name) {
        return execute(() -> restClient.get().uri(String.format("%s/test?name=%s", URL, name)).retrieve().body(String.class));
    }
    ...
    public String findOneGame(Integer id) {
        return execute(() -> restClient.get().uri(String.format("%s/game/%s", URL, id)).retrieve().body(String.class));
    }
