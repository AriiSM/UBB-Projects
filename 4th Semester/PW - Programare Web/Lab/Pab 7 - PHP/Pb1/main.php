<?php
    ...
    function get_trains($from, $to) {
		...
		if (isset($_GET['direct'])) {
			$stmt = $conn->prepare('SELECT * FROM trenuri WHERE start = ? AND stop = ?');
		...
		if (isset($_GET['inter'])) {
			$stmt = $conn->prepare('SELECT T1.trainNo, T1.start, T1.stop, T1.dHour, T1.aHour, T2.stop as stop2,
											T2.dHour as dHour2, T2.aHour as aHour2 FROM trenuri T1 INNER JOIN 
											trenuri T2 ON T1.stop = T2.start  
...
<body>
	<fieldset>
		<form method="get" action="main.php">
			<label for="start">Plecare: </label>
			<input type="text" name="start"><br>
			<label for="stop">Sosire: </label>
			<input type="text" name="stop" id="stop"><br>
			<label>Tip:</label>
			<input type="checkbox" name="direct">Directa</input>
			<input type="checkbox" name="inter">Cu legatura</input><br>
